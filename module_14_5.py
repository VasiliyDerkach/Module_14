...
async def get_buying_list(message):
    ...
    prods = get_all_products(cursr)
    ikey_buy = InlineKeyboardMarkup()
    for prd in prods:
        with open(f'images/product{prd[0]}.webp','rb') as imge:
            await message.answer_photo(imge,f'Название: {prd[1]} | Описание:  {prd[2]} | Цена: {prd[3]}')
        butt_b1 = InlineKeyboardButton(text=prd[1], callback_data='product_buying')
